 ...
 */
public class CommonLoadingAnim extends View {
    private static final String TAG = CommonLoadingAnim.class.getSimpleName();
    ...
    public CommonLoadingAnim(Context context, AttributeSet attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs, R.styleable.zzy_emm_CommonLoadingAnim));
...
//        }
    ...
    private void setupBounds(int layout_width, int layout_height) {
        ...
        if (!fillRadius) {
            ...
            circleBounds =
                    new RectF((float) xOffset + barWidth, (float)yOffset + barWidth, (float)xOffset + circleDiameter - barWidth,
                            (float)yOffset + circleDiameter - barWidth);
        } else {
            circleBounds = new RectF((float)paddingLeft + barWidth, (float)paddingTop + barWidth,
                    (float)layout_width - paddingRight - barWidth, (float)layout_height - paddingBottom - barWidth);
        }
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius =
                (int) a.getDimension(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_circleRadius, circleRadius);

        fillRadius = a.getBoolean(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_fillRadius, false);

        barWidth = (int) a.getDimension(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_barWidth, barWidth);

        rimWidth = (int) a.getDimension(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_rimWidth, rimWidth);

        float baseSpinSpeed =
                a.getFloat(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime =
                a.getInt(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_barSpinCycleTime, (int) barSpinCycleTime);

        barColor = a.getColor(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_barColor, barColor);

        rimColor = a.getColor(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_rimColor, rimColor);

        linearProgress = a.getBoolean(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_linearProgress, false);

        if (a.getBoolean(R.styleable.zzy_emm_CommonLoadingAnim_zzy_emm_matProg_progressIndeterminate, false)) {
            spin();
        }
...
//            }
    ...
    public void setInstantProgress(float progress) {
        ...
        if (Math.abs(progress - mTargetProgress) < 0.01) {
            return;
        }
    ...
    public void setProgress(float progress) {
        ...
        if (Math.abs(progress - mTargetProgress) < 0.1) {
            return;
        }
